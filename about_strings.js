...
exports.strings_behave_like_arrays_of_character_strings = function(test) {
	...
	test.equals(undefined, mango[5]); // Off the edge
...
exports.strings_subarrays_with_substring = function(test) {
	var name = "Joe Bloggs";
	test.equals('J', name.substring(0, 1));
	test.equals('Joe Bloggs', name.substring(0, 100));
	test.equals('Joe', name.substring(3, 0));
	test.equals('Bloggs', name.substring(4, 10));
	test.equals('Bloggs', name.substring(10, 4));
	test.equals('', name.substring(0, -1));
	test.done();
};

exports.strings_subarrays_with_substr = function(test) {
	var name = "Joe Bloggs";
	test.equals('J', name.substr(0, 1));
	test.equals('Joe Bloggs', name.substr(0, 100));
	test.equals('', name.substr(3, 0));
	test.equals('Bloggs', name.substr(4, 10));
	test.equals('', name.substr(10, 4));
	test.equals('', name.substr(0, -1));
	test.equals('Bloggs', name.substr(4));
	test.done();
};
